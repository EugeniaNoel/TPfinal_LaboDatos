...
#%%
#region df
df = pd.read_pickle('/home/ingrid/Documents/labodatos/TP_final/df_principal/df_principal.pkl').drop(
    columns=['Africa','Australia','Eurasia','North America','South America','Country_coord','Country_lat','Country_long',
    'Papunesia','Country_name'])
...
geodf_native = gpd.GeoDataFrame(df,geometry='point_lang').drop(columns=['Latitude_lang','Longitude_lang'])
...
query = pd.read_csv('/home/ingrid/Documents/labodatos/TP_final/df_principal/geoloc/query_coord.csv'
...
ax = world[world.continent == "South America"].plot(edgecolor="black", figsize=(10, 10),
    column='name', cmap='Greens'
)

# We can now plot our ``GeoDataFrame``.
area_select = geodf_native[geodf_native.Macroarea == "South America"]

area_select.plot(ax=ax, marker="o", markersize=15,alpha=0.5,column='num_speakers',legend=True)
...
ax = world.plot(edgecolor="black", figsize=(10, 10),
    column='continent'
)
...
# We restrict to South America.
ax = world[world.continent == "North America"].plot(edgecolor="black", figsize=(8,8),
    color='white'
)
...
area_select.plot(ax=ax, marker="o", markersize=30,alpha=0.5,column='num_speakers',cmap='plasma_r',
...
#endregion
# %%
ax = world[world.continent == "Africa"].plot(edgecolor="black", figsize=(8,8),
    color='white'
)


# We can now plot our ``GeoDataFrame``.
area_select = geodf_native[geodf_native.Macroarea == "Africa"].sort_values('num_speakers')

#ax.set_xlim(-45, 200)
area_select.plot(ax=ax, marker="o", markersize=30,alpha=0.3,column='num_speakers',cmap='plasma_r',legend=True)
